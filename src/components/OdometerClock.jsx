            ...
            >
                {digits.concat(digits).map((digit, index) => (
                    <div
                        key={index}
                        className={cn(
                            "flex items-center justify-center w-full h-16 text-7xl font-normal text-white",
...
const OdometerClock = () => {
    ...
    const [countdownTarget, setCountdownTarget] = useState(() => {
        const saved = JSON.parse(localStorage.getItem(COUNTDOWN_TIME));
        return saved ? new Date(saved) : new Date().setHours(17, 15, 0);
    });
    ...
    useEffect(() => {
        const timer = setInterval(() => {
            ...
            if (isCountDown && countdownTarget && !hasNotified) {
                const timeRemaining =
                    new Date(countdownTarget).getTime() - newTime.getTime();
                const wasPositive =
                    new Date(countdownTarget).getTime() - time.getTime() > 0;
    ...
    useEffect(() => {
        ...
        if (savedTarget) {
            ...
            setCountdownTarget(new Date().setHours(17, 15, 0));
        }
